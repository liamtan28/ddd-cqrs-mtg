import { v4 } from "uuid";
import { MONGO_DB_USER, MONGO_DB_PASS } from "../../credentials";
/*
...
for (const card of cards ) {
    const legalFormats = [];
    for (const [key, value] of Object.entries(card.legalFormats)) {
        if (value === "legal") {
            legalFormats.push(key.charAt(0).toUpperCase() + key.slice(1))
        }
    }
    card.legalFormats = legalFormats as any;
    cardsProcessed.push(card);
}

console.log(JSON.stringify(cardsProcessed, null, 2));
